...
def solution(a)
  a.unshift(1) # Donde estamos parados inicialmente (a[0])
  n = a.size
  positions = []
  fib = fibo(n)

  min_distance = [2<<256]*n
  ...
  for i in (n-1).downto(0) do
    ...
    j = 0
    while fib[j] < n + 1 - i do
      if a[i+fib[j]] == 1
        min_distance[i] = [min_distance[i+fib[j]] + 1, min_distance[i]].min
        break if min_distance[i] == 2
      end
      j+= 1
    end
  end
  min_distance[0] == 2<<256 ? -1 : min_distance.first
end
